...
{
local bash_cmd="bash"; local oifs v vl; local vars="$1"; shift 1; 
local cmds="$*";oifs=$IFS
export IFS=","
for v in $vars; do
export IFS="$oifs"
...
for i in ${cchk}; do
#echo -----------------------------
v1=`distr_runtime_query $i`;eval $v1;unset v1
...
# change variable names only, not content
 ...
 echo -n ${out1/$v2/runtime_rt_v}; echo -n "=";
 generic_cut_param "=" 2 "$s"; echo
done
#exit
}
...
distr_cli_status
echo "---------- help -----------"
echo "  status - show this installation status"
echo "  cmds-check   - check/show cmd path"
echo "  vars   - show system variables"
echo "  module-list   - show system variables"
echo "  runtime-which   - \"which\" in runtimes"
echo "  runtime-query   - query cmd in runtimes"
#echo "  module-list   - "
}
...
if [ x${cmd} == x""  ]; then echo -n; distr_cli_help; fi
