...
main_mod_version_stamp_hlpr() {
  ...
  local var3=$3
  local var4=$4
  local res #echo $var1*100000+1000*$var3+$var4
  let res=1000000*var1+1000*var3+var4
  echo $res
}
...
main_find_module_dir() {
  ...
  local f
  if [ x$name == x ]; then
    echo "main_find_module_dir : module name not set"
    exit
  fi
...
main_enable_one_mod() {
  ...
  if [ x$enable_one_mod_name == x ]; then
    echo "main_enable_one_mod : name not set"
    exit
  fi
  ...
  if [ x${_ret} == x0 ]; then
    ...
    echo "main_enable_one_mod : no such module \"$enable_one_mod_name\""
    return
  fi
...
main_disable_one_mod() {
  ...
  if [ x$disable_one_mod_name == x ]; then
    echo "main_disable_one_mod : name not set"
    exit
  fi
  moddir=MODINFO_modpath_${disable_one_mod_name}
  if [ x${!moddir} == x ]; then
    echo "main_disable_one_mod : no such module"
    return
  fi
