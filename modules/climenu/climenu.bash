...
{
  if nodecfg_nodeid_exists "$n"; then dbg_echo climenu 4  "found NODEID $n" ; return 0; fi
  if hostcfg_hostid_exists "$n"; then dbg_echo climenu 4  "found HOSTID $n" 1>&2; return 0; fi
...
d=${_file0}
while true; do
  d=`dirname $d`;n=`basename $d`
  dbg_echo climenu 14 "F n=$n" 1>&2
  if nodecfg_nodeid_exists "$n"; then dbg_echo climenu 4  "found NODEID $n" 1>&2; break; fi
  if hostcfg_hostid_exists "$n"; then dbg_echo climenu 4  "found HOSTID $n" 1>&2; break; fi
...
for m in $MODULE_list_enabled ; do
  ...
  echo -n
  #dbg_echo climenu 4 "F $f() not found"
...
climenu_cli_help()
{
dgridsys_s;echo "climenu CMDONE - <xxx> <yyy> .... -"
dgridsys_s;echo "climenu CMDTWO - <xxx> <yyy> .... -"
}
