...
{
if [ "x$*" == "x" ]; then echo "run_nodecmd: need parameters"; exit; fi
...
{
# snippet types: 
# bashfunc, nodecmd , nodecmdshell, hostcmd
...
#if [ "x$snp_type" == "xnodecmd" -o "x$snp_type" == "xnodecmdshell" ]; then
dbg_echo run 4 this_NODE_INSTPATH=$this_NODE_INSTPATH
#if [ "x$snp_type" == "xhostcmd" ]; then if [ "x$snp_hostid" == "" ]; then
...
export CONNECT_dnsname=${rmt_CONNECT_dnsname}
...
echo -n
#set -x
(cd $CONNECT_wdir ; env -i HOME=$HOME \
  LC_CTYPE="${LC_ALL:-${LC_CTYPE:-$LANG}}" PATH="$PATH" USER="$USER" \
  $RUN_bash -l -c "$r_cmd ${params}" )
#set +x
else
