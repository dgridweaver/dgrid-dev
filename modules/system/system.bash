...
system_env_start() {
  ...
  export system_trans_log_dir=$(dcmd_dirname $system_trans_log)
...
function system_trans_transdir { echo $(system_trans_localdataprefix)/trans/; }
function system_trans_transfile {
  local trid=$1
  echo $(system_trans_transdir)/$trid
}
...
function system_trans_genid {
  ...
  echo $(date +%s-%N)-$(generic_randompass 16)
}
...
system_trans_register() { # [API]
  ...
  if [ -z "$files" ]; then
    dbg_error "ERROR!"
    dbg_error_echo "No file list, aborting!"
    return 1
  fi
...
system_register_file_changes() { # [API]
  local F0=$1
  local F1=$2
  shift 2
  local f=$*
  if [ -z "$1" ]; then
    distr_error "Need 3 parametes"
    return
  fi
  local trid=$(system_trans_genid)
  dbg_echo system 6 system_trans_begin $trid system $F0 $F1
  system_trans_begin $trid system $F0 $F1
  dbg_echo system 6 system_trans_register $trid system $F0 $f
  system_trans_register $trid system $F0 $f
  dbg_echo system 6 system_trans_end "$trid" system $F0
  system_trans_end "$trid" system $F0
}
...
function system_enable_one_mod {
  ...
  if [ x${!modenabled} == "xY" ]; then
    echo "system_enable_one_mod : \"$system_enable_one_mod_name\" already enabled"
    return
  fi
  ...
  local _ret=$?
  if [ "x${_ret}" != "x0" ]; then
    echo "system_enable_one_mod : no such module \"$system_enable_one_mod_name\""
    return
...
function system_disable_one_mod {
  ...
  if [ x${!moddir} == x ]; then
    echo "system_disable_one_mod : no such module \"$system_disable_one_mod_name\""
    return
  fi

  local modenabled=MODINFO_enable_${system_disable_one_mod_name}
  if [ x${!modenabled} != "xY" ]; then
    echo "system_disable_one_mod : \"$system_disable_one_mod_name\" already disabled"
    return
  fi
